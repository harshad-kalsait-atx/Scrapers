...
class PinterestScraper:
    ...
    def get_similar_pins_from_pin_page(self, main_pin_url, count=30):
        ...
        with sync_playwright() as playwright:
            ...
            try:
                self.logger.debug(f"Navigating to pin page: {main_pin_url}")
                page.goto(main_pin_url, timeout=30000)
    ...
    def collect_all_main_pins(self, keyword, main_count=5):
        """Collect main pins"""
        ...
        if not main_pins:
            ...
            return []
        self.logger.info(f"✅ Found {len(main_pins)} main pins")
        all_main_pin_urls = set(main_pins)  # Start with main pins
        return list(all_main_pin_urls)

    def collect_all_main_and_similar_pins(self, keyword, main_count=5, similar_count=5, extract_similar=True):
        ...
        if extract_similar:
            self.logger.info(f"🔍 STEP 2: Getting similar pins from {len(all_main_pin_urls)} main pins")    
            print(f"\n🔍 STEP 2: Getting similar pins from each main pin")

            for main_pin_url in all_main_pin_urls:
                pin_id = self.extract_pin_id_from_url(main_pin_url)
                self.logger.info(f"Processing main pin: {pin_id}")
                print(f"   📌 Processing main pin: {pin_id}")

                # Get similar pins from the main pin page
                similar_pins = self.get_similar_pins_from_pin_page(main_pin_url, similar_count)

                before_count = len(all_main_pin_urls)
                all_main_pin_urls.update(similar_pins)
                after_count = len(all_main_pin_urls)
            ...
            self.stats['similar_pins_found'] = len(all_main_pin_urls) - len(main_pins)
        else:
        ...
        self.stats['total_unique_pins'] = len(all_main_pin_urls)

        self.logger.info(f"✅ Total unique pins collected: {len(all_main_pin_urls)}")
        print(f"\n✅ Collection complete:")
        print(f"   📊 Main pins: {len(main_pins)}")
        print(f"   📊 Similar pins: {self.stats['similar_pins_found']}")
        print(f"   📊 Total unique pins: {len(all_main_pin_urls)}")
        self.logger.info(f"Total unique pins collected: {len(all_main_pin_urls)}")
        return list(all_main_pin_urls)
    ...
    def run(self, keyword, main_count=5, similar_count=5, extract_similar=True):
        ...
        try:
            ...
            if extract_similar == True:
                all_pin_urls = self.collect_all_main_and_similar_pins(keyword,main_count, similar_count, extract_similar)
                print("_____________________________________________________________________________________")
            else:
                all_pin_urls = self.collect_all_main_pins(keyword, main_count)
...
if __name__ == "__main__":
    ...
    # Interactive prompt for similar pins
    ...
    scraper.run(keyword, main_count, similar_count, extract_similar)
