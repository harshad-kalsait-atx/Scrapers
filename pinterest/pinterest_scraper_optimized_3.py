...
class PinterestScraper:
    def __init__(self, zoom_level=0.25):
    ...
    def wait_for_page_load(self, page, timeout=20000):
            ...
            try:
                ...
                while new_pins_found < count and scroll_count < max_scrolls:
                    ...
                    if current_pin_count == previous_pin_count:
                        stable_count += 1
                        if stable_count >= 5:  # Increased threshold
...
def get_user_input():
    ...
    print(f"\n🔍 Zoom Level Configuration:")
    print("Lower zoom = more pins visible, higher zoom = better detail")
    print("Recommended: 25% (0.25) for maximum efficiency")
    zoom_input = input("Enter zoom level (10-100%, default: 25): ").strip()
    
    if zoom_input:
        try:
            zoom_percent = int(zoom_input)
            if 10 <= zoom_percent <= 100:
                zoom_level = zoom_percent / 100
            else:
                print("Invalid zoom level, using default 25%")
                zoom_level = 0.25
        except ValueError:
    ...
    return keyword, main_count, similar_count, zoom_level

if __name__ == "__main__":
    keyword, main_count, similar_count, zoom_level = get_user_input()

    scraper = PinterestScraper(zoom_level=zoom_level)
